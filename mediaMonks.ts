...
pages.forEach((page) => {
  page.addEventListener("click", () => {
    const selectedPage: number = parseInt(page.getAttribute("pageNumber")); // Parsing the attribute from string to match the variable type.
...
const renderPageData = (data: any) => {
  ...
  const subtitleText =
    data.subtitle != null ? data.subtitle : stepsSubtitle(data.id);
  const subtitleClass =
    data.subtitleClass != null ? data.subtitleClass : stepsSubtitlePosition;
...
const getLastClass = (element: any): string => {
  let lastClasses: string[] = element.classList.value.split(" ");
  let lastClass: string = lastClasses.length > 1 ? lastClasses.pop() : null;
  return lastClass;
};

const stepsSubtitle = (page) =>
  `Step ${page} out of 8 on the path to digital enlighment`;
...
const pageData: any[] = [
  {
    ...
    titleClass: "topLeft",
    subtitle:
      "In January 2011, after a decade of digital, we opened the doors to our temple.<br>Follow our noble eightfold path to digital enlightment here.",
  ...
  },
  {
    ...
    titleClass: "topRight",
    subtitle:
      "Interested in joining our monastery?<br> Check out our current openings on <a>mediamonks.com/careers</a><br><br><span class='mainScreen-social'><i class='lni lni-envelope'></i>Mail ons <i class='lni lni-facebook-filled'></i> Facebook <i class='lni lni-twitter-filled'></i> Twitter</span>",
